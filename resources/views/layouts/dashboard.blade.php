...
<head>
    ...
    <title>@yield('title', 'Dashboard') - KosKu</title>
...
<div class="dashboard-shell">
    ...
    @endif
    <div class="dashboard-main {{ auth()->user()->role === 'user' ? 'full' : '' }}">
        @php
        ...
        <div class="dashboard-topbar professional-topbar">
            <div class="d-flex align-items-center gap-3">
                <a href="{{ route('home') }}" class="btn btn-light btn-sm topbar-home"><i class="bi bi-house"></i></a>
                <div>
            ...
            <div class="dropdown dashboard-profile-dropdown">
                <button class="dashboard-profile-btn dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                    <span class="dashboard-profile-avatar"><i class="bi {{ $roleIcon }}"></i></span>
                    <span><strong>{{ auth()->user()->name }}</strong><small>{{ $roleLabel }}</small></span>
                </button>
                <div class="dropdown-menu dropdown-menu-end shadow-sm dashboard-profile-menu">
                    <div class="dashboard-profile-head">
                        <span class="dashboard-profile-avatar lg"><i class="bi {{ $roleIcon }}"></i></span>
                        <div><strong>{{ auth()->user()->name }}</strong><small>{{ auth()->user()->email }}</small><em>{{ $roleLabel }}</em></div>
                    </div>
                    ...
                    <a class="dropdown-item" href="{{ route('kos.index') }}"><i class="bi bi-window-stack me-2"></i>Lihat Website</a>
